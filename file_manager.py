...
def changedir(inp):
    ...
    if os.path.isdir(value) == True:
        ...
        sd.err('TERMINAL','Logic', f"Directory '{value}' does not exist.")
...
def dir_table(dirs, path):
    ...
    for i in dirs:
        try:
            ...
            sd.err('TERMINAL', 'Permission', f'Cannot access {i} in {path}.')
...
def checfunc(text, status):
    ...
    if status == True:
        print(f'[{Fore.GREEN}OPERATIONAL{Fore.WHITE}]')
        return 'operational'
    else:
        print(f'[{Fore.RED}MISSING{Fore.WHITE}]')
...
def terminalReadInput(inp):         #Terminal (main)
    ...
    if command == 'writein':
        ...
        if feedback == '':
            feedback = 'No missing or corrupted files found.'
            sd.msg('TERMINAL', 'success', feedback)
        else:
