...
if __name__ == "__main__":
    os.system('cls')
    state = fm.terminalReadInput('chmodule')
    old_state = state

    sd.msg(state, 'success', f"Welcome! Terminal Basic {state} module engaged. Type 'commandlist' inside module for module command list.")
    ...
    while True:
            ...
            if state == 'MENU ABANDONED, NO DECISION MADE.' or state == 'EXIT':
                os.system('cls')
                sd.msg("TERMINAL", 'warning', 'Menu abandoned, no dicision made. Exiting Terminal Basic...')
                sleep(2)
                exit()
